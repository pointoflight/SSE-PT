...
class Model():
    def __init__(self, usernum, itemnum, hparams, reuse=tf.AUTO_REUSE):
        self.is_training = tf.placeholder(tf.bool, shape=())
        self.u = tf.placeholder(tf.int32, shape=(None))
        self.input_seq = tf.placeholder(tf.int32, shape=(None, hparams[HP_MAXLEN]))
        self.pos = tf.placeholder(tf.int32, shape=(None, hparams[HP_MAXLEN]))
        self.neg = tf.placeholder(tf.int32, shape=(None, hparams[HP_MAXLEN]))
        pos = self.pos
        neg = self.neg
        ...
        with tf.variable_scope("SASRec", reuse=reuse):
            ...
            self.seq, item_emb_table = embedding(self.input_seq,
                                                 ...
                                                 num_units=hparams[HP_ITEMHIDDEN],
                                                 zero_pad=True,
                                                 scale=True,
                                                 l2_reg=hparams[HP_L2],
            ...
            t, pos_emb_table = embedding(
                ...
                vocab_size=hparams[HP_MAXLEN],
                num_units=hparams[HP_ITEMHIDDEN] + hparams[HP_USERHIDDEN],
                zero_pad=False,
                scale=False,
                l2_reg=hparams[HP_L2],
                scope="dec_pos",
            ...
            u0_latent, user_emb_table = embedding(self.u[0],
                                                 ...
                                                 num_units=hparams[HP_USERHIDDEN],
                                                 zero_pad=False,
                                                 scale=True,
                                                 l2_reg=hparams[HP_L2],
            ...
            self.hidden_units = hparams[HP_ITEMHIDDEN] + hparams[HP_USERHIDDEN]
            self.seq = tf.reshape(tf.concat([self.seq, self.u_latent], 2),
                                  [tf.shape(self.input_seq)[0], -1, self.hidden_units])
            self.seq += t
            # Dropout
            self.seq = tf.layers.dropout(self.seq,
                                         rate=hparams[HP_DROPOUT],
                                         training=tf.convert_to_tensor(self.is_training))
            self.seq *= mask

            # Build blocks
            self.attention = []
            for i in range(hparams[HP_NUMBLOCKS]):
                with tf.variable_scope("num_blocks_%d" % i):
                    ...
                    self.seq = feedforward(normalize(self.seq), num_units=[self.hidden_units, self.hidden_units],
                                           dropout_rate=hparams[HP_DROPOUT], is_training=self.is_training)
                    self.seq *= mask

            self.seq = normalize(self.seq)
        
        user_emb = tf.reshape(self.u_latent, [tf.shape(self.input_seq)[0] * hparams[HP_MAXLEN], 
                                              hparams[HP_USERHIDDEN]])

        pos = tf.reshape(pos, [tf.shape(self.input_seq)[0] * hparams[HP_MAXLEN]])
        neg = tf.reshape(neg, [tf.shape(self.input_seq)[0] * hparams[HP_MAXLEN]])
        ...
        seq_emb = tf.reshape(self.seq, [tf.shape(self.input_seq)[0] * hparams[HP_MAXLEN], self.hidden_units])
        ...
        self.test_logits = tf.reshape(self.test_logits, [tf.shape(self.input_seq)[0], hparams[HP_MAXLEN], 101])
        ...
        istarget = tf.reshape(tf.to_float(tf.not_equal(pos, 0)), [tf.shape(self.input_seq)[0] * hparams[HP_MAXLEN]])
        ...
        with tf.variable_scope("testAdam", reuse=reuse):
            self.global_step = tf.Variable(0, name='global_step', trainable=False)
            self.optimizer = tf.train.AdamOptimizer(learning_rate=hparams[HP_LR], beta2=0.98)
